...
4. `capture.yaml`을 쓰고 `pnpm capture <slug>` 실행 → 출력된 YAML을 meta.yaml에 붙여 alt 작성. 로컬 실행 레포는 `local: { cwd, command, port }`. 90일 넘은 스크린샷은 `pnpm capture <slug> --stale 90`. 1.5MB 이하.
