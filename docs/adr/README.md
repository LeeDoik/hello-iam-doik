...
- 0005, 0008, 0009는 계획된 결정(스크린샷 파이프라인, hero 3D, OG 이미지)을 위해 미리 예약했던 번호이고, 그 파일명 그대로 손으로 작성해 지금은 모두 채워져 있다. `pnpm adr`는 현재 최대 번호+1로 매기므로, 앞으로 번호를 예약하려면 같은 방식으로 파일을 직접 만든다.
